...
export const dataMap = <T extends ObjType = ObjType>(
  obj: T,
  prefixCls: string
): Ret<T> => {
  const cls = {} as Ret<T>
  keys(obj).forEach((key) => {
    ...
    cls[getAttrName(key, prefixCls) as keyof Ret<T>] = falsely(obj[key])
      ? false
      : obj[key]!.toString()
  })
...
export const usePrefixCls = (cls: string, prefix = 'flex') => {
  return [prefix, cls].filter((s) => s).join('-')
}
