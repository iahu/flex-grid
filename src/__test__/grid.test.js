...
describe('<Grid />', () => {
  ...
  it('should render nested', () => {
    ...
    expect(container.querySelector('.flex-grid > .flex-grid > .flex-grid')).not.toBeNull();
  });
  ...
  it('should make css custom properties', () => {
    ...
    expect(grid.style.getPropertyValue('--flex-grid-align-items')).toBe('center');
    expect(grid.style.getPropertyValue('--flex-grid-align-content')).toBe('center');
  });
});
